...
// FormBuilder class — each instance manages one form on the page.

export class FormBuilder {
    constructor(root) {
        if (!(root instanceof HTMLElement)) {
            throw new Error("FormBuilder requires a root HTMLElement");
        }
    ...
    async _handleSubmit() {
        ...
        if (isMultiStage(this.activeSchema)) {
            const stageErrors = validateStage(this.activeSchema, this.state, this.currentStage);
        ...
        this._pruneHiddenFields();
        const errors = validateStage(this.activeSchema, this.state);
    ...
    _bindEvents() {
        ...
        if (this.nextButton) {
            this.nextButton.onclick = () => {
                ...
                const errors = validateStage(this.activeSchema, this.state, this.currentStage);
                if (Object.keys(errors).length) {
                    this._showErrors(errors);
                    return;
                }
